...
4. **TradingView visual study — REQUIRED.** Confirm the article's rules on 2–3 representative symbols via the connected browser (Claude in Chrome / Cowork computer-use / Playwright real Chrome), apply the article's indicators on its timeframe, screenshot into `artifacts/YYYY-MM-DD/tv/`. If no browser is connected, **stop and ask the user to connect one**; leave `indicator_study` `PENDING`. Never fabricate screenshots.
