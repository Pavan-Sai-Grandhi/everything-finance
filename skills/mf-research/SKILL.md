...
Render `assets/fund-comparison.html` (bundled with this skill) (rolling-return table, drawdown, cost comparison; benchmark row's own "consistency" cell is "—"), save to `artifacts/YYYY-MM-DD/mf-research.html`, summarize the verdict in chat. When comparing two active equity funds, include a portfolio-overlap line (top-holdings intersection %) if holdings data was obtainable. Note data gaps (e.g., fund too young for 5Y rolling). End with: "Not investment advice — personal research tool."
