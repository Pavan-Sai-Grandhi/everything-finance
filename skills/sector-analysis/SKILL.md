...
**Seed the shared cache.** This skill is the canonical refresher of the monthly sector cache that `deep-analysis` reads, so for every sector you analyze, write its **sector-level body** to `paths.sector_cache_path(sector)` with frontmatter `generated: <today>` + `rs_class: <classification>`, and set the next-refresh reminder via `lib/alerts.py`: `kind: sector_refresh_due`, `subject: {type: sector, id: <sector>}`, `trigger: {due: <today + 30d>}`, `created_by: sector-analysis`, `severity: watch`, `action.suggest: "/sector-analysis <sector>"`, `dedup_key: sector-refresh-<sector>` (dedup updates any existing reminder in place). A later deep-analysis on a stock in this sector then reuses this body instead of recomputing it.
