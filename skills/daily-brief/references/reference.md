...
- Watchlist/holdings filings: via the bundled `filings-watch/scripts/filings.py --scrip <BSE_CODE>` (it wraps the BSE public JSON API — browser UA + `Referer: https://www.bseindia.com/`; a 200 "No Record Found!" everywhere = fingerprint block, move on) → screener.in company Documents section as the reliable fallback.
