...
Detect which broker MCP is connected by checking for its tools:
- **Zerodha** → `mcp__kite__*` (hosted `https://mcp.kite.trade/mcp`)
- **Upstox** → `mcp__upstox__*` (hosted `https://mcp.upstox.com/mcp`)

If the user passed `broker:kite|upstox`, use that one. If **neither** is connected, stop and tell the user to authenticate: the plugin declares both servers in `plugin.json`; they log in through the MCP's OAuth (Kite needs a login per session; Upstox once per day). Do not proceed without a real connection — never fabricate positions.

Pull, via the connected MCP:
- **Holdings** (delivery/long-term) and **Positions** (intraday/F&O) — symbol, qty, average price, last price, P&L.
- Recent **orders/trades** if the rationale needs the actual fill price/date.
